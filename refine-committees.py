...
def checkon(fn, o):
    ...
    if True:
        ...
        o.json['roles'] = sorted(o.json['roles'] + [r for r in roles[o.getKey()] if r not in o.json['roles']],
                                 key=lambda x: x[1])
...
if __name__ == "__main__":
    ...
    for line in f.readlines():
        ...
        roles[v].append([n, r])
    f.close()
    print('Metadata on {} editions loaded with {} role assignments'.format(C.red(len(roles)), C.red(
        sum([len(roles[k]) for k in roles.keys()]))))
    # now add
    cx = {0: 0, 1: 0, 2: 0}
